  ...
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <tbody className="bg-white dark:bg-[#0d1421] divide-y divide-gray-200 dark:divide-gray-800">
          {assets.map((asset) => (
            <tr key={asset.id} className="hover:bg-gray-50 dark:hover:bg-gray-900">
              <td className="px-3 py-4 whitespace-nowrap">
                <div className="flex items-center">
                  <button className="text-gray-300 dark:text-gray-600 hover:text-yellow-400 mr-2">
                    <Star className="h-4 w-4" />
                  </button>
                  <span className="text-sm text-gray-500 dark:text-gray-400">{asset.rank}</span>
                </div>
              </td>
              <td className="px-3 py-4 whitespace-nowrap">
                <Link to={`/crypto/${asset.id}`} className="flex items-center hover:bg-gray-50 dark:hover:bg-gray-900">
              ...
              <td
                className={`px-3 py-4 whitespace-nowrap text-sm text-right ${
                  asset.change1h >= 0 ? "text-green-600 dark:text-green-500" : "text-red-600 dark:text-red-500"
                }`}
              >
              ...
              <td
                className={`px-3 py-4 whitespace-nowrap text-sm text-right ${
                  asset.change24h >= 0 ? "text-green-600 dark:text-green-500" : "text-red-600 dark:text-red-500"
                }`}
              >
              ...
              <td
                className={`px-3 py-4 whitespace-nowrap text-sm text-right ${
                  asset.change7d >= 0 ? "text-green-600 dark:text-green-500" : "text-red-600 dark:text-red-500"
                }`}
              >
